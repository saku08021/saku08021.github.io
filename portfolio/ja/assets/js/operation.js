$(document).ready(function() {
  ...
  // 経歴キューブを作成
  const careerCubeGroup = new THREE.Group(); // キューブをまとめるグループ
  const cubeSize = 3;
  const cubeSpacing = 5;

  careerData.forEach((data, index) => {
    const cubeGeometry = new THREE.BoxGeometry(cubeSize, cubeSize, cubeSize);
    const cubeMaterial = new THREE.MeshLambertMaterial({ color: 0x00a8ff }); // 鮮やかな青
    ...
    cube.position.y = index * (cubeSize + cubeSpacing) - (careerData.length * (cubeSize + cubeSpacing) / 2); // キューブを縦に配置
  ...
  const achievementData = [
    ...
    { label: 'SaaSツール開発', value: 1, unit: 'リリース' } // リリースは数値化が難しいので1として表現
  ];
  ...
  const barDepth = 1;
  const barSpacing = 4;
  const maxBarHeight = 10; // 最大のバーの高さ
  ...
  achievementData.forEach((data, index) => {
    ...
    loader.load('https://threejs.org/examples/fonts/helvetiker_regular.typeface.json', function (font) {
      ...
      labelMesh.position.set(-(barWidth + 2), 0, 0); // バーの左側に配置
      bar.add(labelMesh);
      ...
      valueMesh.position.set(0, barHeight / 2 + 0.5, 0); // バーの上に配置
      bar.add(valueMesh);
    });

    bar.position.x = index * (barWidth + barSpacing) - (achievementData.length * (barWidth + barSpacing) / 2); // バーを横に並べる
    achievementBarGroup.add(bar);
  });

  achievementBarGroup.position.set(0, -10, 0); // 下側に配置
  ...
  skillData.forEach((data, index) => {
    ...
    const iconMaterial = new THREE.MeshBasicMaterial({ 
      color: 0xffffff, 
      transparent: true, 
      opacity: data.level / 10 // 光の強弱でレベルを表現
    });
    const icon = new THREE.Mesh(iconGeometry, iconMaterial);

    // Font Awesomeのアイコンを設定 (HTMLの<i>タグと同じクラス名)
    icon.name = data.name; // 後でオブジェクトを識別するために名前を設定
    icon.userData.html = `<i class="${data.icon} fa-3x"></i>`; // HTMLをデータとして格納

    icon.position.x = index * (iconSize + skillSpacing) - (skillData.length * (iconSize + skillSpacing) / 2); 
    skillGroup.add(icon);
  });

  skillGroup.position.set(0, 10, 0); // 上側に配置
